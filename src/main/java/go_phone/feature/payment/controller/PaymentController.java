...
import go_phone.feature.payment.dto.request.PayCreateRequest;
...
public class PaymentController {
    ...
    @Value("${app.payment.return-path}") private String returnPath;

    @PostMapping(ApiConstants.Payment.CREATE)
    public ResponseEntity<ApiResponse<Object>> create(@RequestParam("orderCode") Long orderCode) throws Exception {
        Order o = orderService.findByOrderCode(orderCode);
        int amount = o.getGrandTotal().intValue();

        ItemData item = ItemData.builder()
                .name("GoPhone #" + orderCode).quantity(1).price(amount).build();
