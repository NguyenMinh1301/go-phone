...
public class PayOSReturnController {
    ...
    public ResponseEntity<ApiResponse<Object>> handleReturn(
        ...
        if (orderCode == null) {
            return ResponseHandler.success("Return from PayOS",
                    java.util.Map.of("message", "Thiếu orderCode", "orderStatus", "UNKNOWN"));
        }
        Order o = orderService.findByOrderCode(orderCode);
        return ResponseHandler.success(java.util.Map.of(
                "message", "Trở về từ PayOS",
                "orderCode", orderCode,
                "orderStatus", o.getStatus()        // PENDING_PAYMENT / PAID / ...
        ));
    }
}
